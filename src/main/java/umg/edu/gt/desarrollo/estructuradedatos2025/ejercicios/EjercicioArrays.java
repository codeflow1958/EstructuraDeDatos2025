...
public class EjercicioArrays {
	...
	*/
    ...
    // Ejercicio 1: Encontrar la riqueza máxima de los clientes
    ...
    public static int[][] ordenarDiagonales(int[][] mat) {
        ...
        // Recorrer diagonales que empiezan desde la primera columna
        for (int i = 0; i < m; i++) {
            ordenarDiagonal(mat, i, 0, m, n);
        }
        
        // Recorrer diagonales que empiezan desde la primera fila (excepto la primera ya tratada)
        for (int j = 1; j < n; j++) {
    ...
    private static void ordenarDiagonal(int[][] mat, int row, int col, int m, int n) {
        ...
        while (r < m && c < n) {
            diagonal[index++] = mat[r++][c++];
        }
        
        // Ordenar la diagonal
        Arrays.sort(diagonal);
        
        // Colocar de vuelta los valores ordenados
        r = row; c = col; index = 0;
        while (r < m && c < n) {
